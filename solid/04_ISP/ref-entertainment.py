...
class EtherConnector:
    def connect_to_device_via_ethernet_cable(self, device): pass


class Television(Power, RCAconnector, HDMIconnector):
...
class DVDPlayer(Power, HDMIconnector):
...
class GameConsole(Power, HDMIconnector, EtherConnector):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(Power, EtherConnector):
    def connect_to_tv(self, television):
